...
class ClockView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        if let context = UIGraphicsGetCurrentContext() {
            ...
            context.fillPath()
            // clock's border
            context.addEllipse(in: CGRect(x: rect.origin + borderWidth / 2.0, y: rect.origin.y + borderWidth / 2.0, width: rect.size.width - borderWidth, height: rect.size.height - borderWidth))
            ...
            let centerCircle = CGRect(x: clockCenter.x - largeDotRadius, y: clockCenter, width: 2 * largeDotRadius, height: 2 * largeDotRadius)
